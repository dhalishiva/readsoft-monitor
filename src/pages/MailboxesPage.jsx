...
import {
  Plus, Mail, Bell, Key, Trash2, RefreshCw,
  CheckCircle, Edit2, AlertTriangle
} from 'lucide-react';
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
            Manage your approval mailboxes and token health.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            Add your first mailbox to start monitoring.
          </p>
          <button
            onClick={() => setShowAdd(true)}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-semibold truncate text-slate-900 dark:text-white text-sm">
                      {mb.email}
                    </h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">
                      {mb.client_id?.slice(0, 8)}...
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-2">
                ...
                <div
                  ...
                  title={mb.last_sync_at ? formatFull(mb.last_sync_at) : ''}
                >
                  <p className="text-xs text-slate-500 dark:text-slate-400">Last Sync</p>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">
                    {timeAgo(mb.last_sync_at)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-1 pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  onClick={() => setRegenFor(mb)}
                  className="flex-1 px-2 py-2 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs flex items-center justify-center gap-1.5"
                  title="Regenerate token via Microsoft"
                >
                  <Key size={14} /> Regen
                </button>
                <button
                  onClick={() => setEditFor(mb)}
                  className="px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-400"
                  title="Edit connection details"
                >
                  <Edit2 size={16} />
                </button>
                <button
                  onClick={() => handleMarkRegenerated(mb.id)}
                  className="px-3 py-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg"
                  title="Mark as regenerated in ReadSoft"
                >
                  <CheckCircle size={16} />
                </button>
                <button
                  onClick={() => handleDelete(mb.id)}
                  className="px-3 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-600 rounded-lg"
