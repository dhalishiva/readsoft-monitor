...
import { History, CheckCircle, XCircle, Mail, Key, AlertTriangle, RefreshCw } from 'lucide-react';
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 mb-4 flex gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Filter by Mailbox</label>
          <select
            value={filterMailbox}
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {filtered.map(a => {
              ...
              return (
                <div key={a.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1 ${typeColors[a.alert_type] || 'bg-slate-100 text-slate-700'}`}>
                          <Icon size={11} />
                    ...
                    <div className="shrink-0 text-right">
                      <p className="text-xs text-slate-500 dark:text-slate-400" title={formatFull(a.sent_at)}>
                        {timeAgo(a.sent_at)}
                      </p>
                    </div>
                  </div>
