...
import {
  ...
  MoreVertical, UserCheck, UserX, KeyRound,
} from 'lucide-react';
...
export default function AdminsPage() {
  ...
  const [actionSheet, setActionSheet] = useState(null); // admin id
  ...
  const handleDelete = async (admin) => {
    ...
    try {
      await deleteAdmin(supabase, admin.id);
      setActionSheet(null);
      load();
    } catch (err) {
      alert(err.message);
        ...
        {loading ? (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {admins.map(a => (
                <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors relative">
                  ...
                  <td className="px-5 py-3 text-right">
                    {isSuperAdmin && a.id !== currentAdmin?.id && (
                      <div className="relative inline-block">
                        <button onClick={() => setActionSheet(actionSheet === a.id ? null : a.id)}
                        ...
                        {actionSheet === a.id && (
                          <>
                            ...
                            <div className="absolute right-0 z-20 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg overflow-hidden">
                              ...
                              <button onClick={() => handleToggleActive(a)}
                                className="w-full text-left px-4 py-2.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-2">
                                {a.is_active
                                  ? <><UserX size={14} className="text-amber-500" /> Disable</>
                                  : <><UserCheck size={14} className="text-emerald-500" /> Enable</>
                                }
                              </button>
