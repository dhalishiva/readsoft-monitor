...
function ActionSheet({ admin, isSuperAdmin, isSelf, onClose, onDisable, onDelete, onReset }) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end md:hidden" onClick={onClose}>
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-t-2xl p-4 pb-8 shadow-2xl"
        onClick={e => e.stopPropagation()}>
        <div className="w-10 h-1 bg-slate-300 dark:bg-slate-700 rounded-full mx-auto mb-4" />
        <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-4 truncate px-4">{admin.email}</p>
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
          ...
          {isTargetSuperAdmin && <Shield size={11} />}{a.role}
        </span>
        ...
        {a.approved_at && (
          <span className="text-xs text-slate-400 dark:text-slate-500" title={formatFull(a.approved_at)}>
            Approved {timeAgo(a.approved_at)}
          </span>
        )}
      </div>
      {sheetOpen && (
        <ActionSheet admin={a} isSuperAdmin={isSuperAdmin} isSelf={isSelf}
...
export default function AdminsPage() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault(); setCreateMsg(null); setCreating(true);
    try {
  ...
  const handleDelete = async (admin) => {
    ...
    if (!confirm(isPending ? `Reject request from ${admin.email}?` : `Delete ${admin.email}? Cannot be undone.`)) return;
    try {
      const result = await deleteAdmin(admin.id);
      if (result?.warning) alert(`Removed, but: ${result.warning}`);
      load();
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <h1 className="text-xl md:text-2xl font-bold mb-5 text-slate-900 dark:text-white">Admin Users</h1>
              ...
              <div key={a.id} className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-amber-100 dark:border-amber-900/50">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{a.email}</div>
                    <div className="text-xs text-slate-400 mt-0.5" title={formatFull(a.created_at)}>
                      Requested {timeAgo(a.created_at)}
                    </div>
                  </div>
                </div>
                {a.signup_reason && (
                  <div className="mb-3 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg flex gap-2">
                    <MessageSquare size={14} className="shrink-0 mt-0.5 text-slate-400" />
                    <span className="text-xs md:text-sm text-slate-700 dark:text-slate-300 break-words">{a.signup_reason}</span>
                  </div>
                )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 md:p-6 mb-5">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Skip the approval workflow. Share the password via a secure channel.
        </p>
        <form onSubmit={handleCreate} className="flex flex-col md:grid md:grid-cols-3 gap-3">
          <input type="text" required placeholder="Full name" value={createForm.full_name}
            onChange={updateCreate('full_name')} disabled={creating}
          ...
          <input type="email" required placeholder="email@company.com" value={createForm.email}
            onChange={updateCreate('email')} disabled={creating}
            className="px-3 py-2.5 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 dark:text-white rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 text-sm" />
          <div className="relative">
            <input type={showCreatePassword ? 'text' : 'password'} required minLength={6}
              placeholder="Password (min 6 chars)" value={createForm.password}
              onChange={updateCreate('password')} disabled={creating}
            ...
            <button type="button" onClick={() => setShowCreatePassword(s => !s)}
            ...
            <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-800">
              {approvedAdmins.map(a => (
                <div key={a.id} className="p-4">
                  <AdminCard a={a} currentAdmin={currentAdmin} isSuperAdmin={isSuperAdmin}
                      ...
                      <tr key={a.id}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            {isTargetSuperAdmin && <Shield size={12} />}{a.role}
                          </span>
                        </td>
                        <td className="px-6 py-4">
      ...
      {resetTarget && (
        <ResetPasswordModal target={resetTarget} onClose={() => setResetTarget(null)} onSuccess={load} />
      )}
    </div>
  );
}